...
class SparseBatch(ctypes.Structure):
    ...
    def get_tensors(self, device):
        ...
        w_indices.to(device=device, non_blocking=True)
        b_indices.to(device=device, non_blocking=True)
        

        # Calculer les offsets depuis les index de batch (iw[0] et ib[0])
        w_counts = torch.bincount(iw[0], minlength=self.size)
        w_offsets = torch.zeros(self.size, dtype=torch.long, device=device)
        w_offsets[1:] = w_counts[:-1].cumsum(0)

        b_counts = torch.bincount(ib[0], minlength=self.size)
        b_offsets = torch.zeros(self.size, dtype=torch.long, device=device)
        b_offsets[1:] = b_counts[:-1].cumsum(0)

        # Regrouper chaque couleur sous forme d'un tuple (indices, offsets, values)
        w_data = (w_indices, w_offsets)
        b_data = (b_indices, b_offsets)

        return us, them, w_data, b_data, outcome, score
        ...
        # return us, them, iw, ib, outcome, score
