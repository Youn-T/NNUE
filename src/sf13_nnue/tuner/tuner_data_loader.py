...
class HalfKPDatasetTuning(IterableDataset):
    def __init__(self, data_dir='data/halfkp_data',tuning_data_dir='data/tuning_data', batch_size=8192, shuffle=True):
        """Initialization"""
        super().__init__()
        self.base_file_paths = sorted(glob.glob(os.path.join(data_dir, "*.pt")))
        self.tuning_file_paths = sorted(glob.glob(os.path.join(tuning_data_dir, "*.pt")))
        self.file_paths = self.base_file_paths + self.tuning_file_paths
    ...
    def __iter__(self):
        ...
        for path in files_to_process:
            ...
            if (path in self.tuning_file_paths):
                # wdl = cp_to_wdl(score) # Convert centipawn scores to WDL probabilities for tuning data
                pass
            else :
